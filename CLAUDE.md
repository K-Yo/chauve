...
**`src/settings.rs`** — Loads `Settings.toml` (or `APP_` prefixed env vars as fallback) into strongly-typed structs via `config` + `serde`. Settings are passed to the `Poller` at startup.

**`src/ui/`** — Dioxus components. `app.rs` is the root: it loads settings, creates the `Poller`, and provides it via Dioxus context. `AlertsApp` runs a coroutine that polls every 5 seconds and stores results in a signal. Components consume that signal reactively. Tailwind CSS is used for all styling; severity-specific classes (`.severity-critical`, etc.) are defined in `assets/source.css`.
